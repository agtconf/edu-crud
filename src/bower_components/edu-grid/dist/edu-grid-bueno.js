  ...
  return {
    ...
    link: function ($scope, $filter) {
      ...
      /* $scope.showForm=false;
                if($scope.options.showButtonsCrudPre){
                	$scope.options.buttonsCrudPre= [
                             {label: 'Editar', class: '', glyphicon: 'edit', button: false, onclick: function (row) {
                                 $scope.edit(row);
                             }},
                             {label: 'Eliminar', class: '', glyphicon: 'trash', button: false, onclick: function (row) {
								 $scope.selectedRowForDelete=row;
								 $scope.keyRowForDelete=$scope.options.fieldKeyLabel + ": "+ row[$scope.options.fieldKey]+ "?";
								 $scope.options.showOverlayFormDelete=true;
                             }}
                         ];
                };
                if($scope.options.showButtonsCrudPost){
                	$scope.options.buttonsCrudPost= [
                       {label: 'Editar', class: '', glyphicon: 'edit', button: false, onclick: function (row) {
                           $scope.edit(row);
                       }},
                       {label: 'Eliminar', class: '', glyphicon: 'trash', button: false, onclick: function (row) {
      ...
      for (var fieldKey in $scope.options.listFields) {
        console.log('edu-crud.js   prepare field-render1:' + angular.toJson(fieldKey));
        console.log('edu-crud.js   prepare field-render2:' + angular.toJson($scope.options.listFields[fieldKey]));
