...
.run(function($httpBackend,$http,$log,$filter,filterFilter) {
  ...
  /*	$http.get('json/municipios.json').success(function(data) {
        municipios =data; 
        $log.log("load json municipios:"+data.length);
    });*/
    $http.get('json/centros.json').success(function(data) {
        centros =data.centros; 
        $log.log("load json centros:"+data);
    });
    
    /*$scope.order = function(predicate, reverse) {
        centros = orderBy(centros, predicate, reverse);
    };
    */
	// GET one municipio from municipios array
    /*$httpBackend.whenGET(/api\/v1\/municipios\/([0-9a-zA-Z$_\.\+!\*\'\(\),;:@&=-])+/).respond(function(method, url,data,headers) {
        console.log("llamada a GET api/v1/provincias/:id "+method + " "+ url+ " "+url.split('/')[3]);
        var municipo="";
        for(i=0;i<municipios.length;i++){
            if(municipios[i].id==url.split('/')[3]){
                municipio=municipios[i];
            }
        }
        return [200, municipio, {}];
    });*/
